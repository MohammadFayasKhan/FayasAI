...
static size_t s_bufferCapacity = 0;       // WAV_HEADER_SIZE_BYTES + AUDIO_MAX_BUFFER_BYTES
...
bool begin() {
    ...
    i2s_chan_config_t chanCfg = I2S_CHANNEL_DEFAULT_CONFIG(I2S_PORT_NUM, I2S_ROLE_MASTER);
    ...
    i2s_std_config_t stdCfg = {
        .clk_cfg  = I2S_STD_CLK_DEFAULT_CONFIG(16000), // Run hardware at 16kHz (minimum for INMP441)
    ...
    if (s_buffer == nullptr) {
        Serial.println(F("[Audio] FATAL: could not allocate recording buffer at boot!"));
        return false;
    }
...
bool pump() {
    ...
    // We run I2S hardware at 16kHz but downsample (decimate) by 2 to save to 8kHz.
    // Thus, we read 2x as many samples from I2S as we write to the destination.
    // Slot width is 32-bit (4 bytes), destination is 16-bit (2 bytes).
    // So 1 sample in I2S = 4 bytes, 1 sample in destination = 2 bytes.
    // 2 samples read from I2S (8 bytes) -> 1 sample written to destination (2 bytes).
    // Therefore, bytesToRequest from I2S is remainingCapacity * 4.
    size_t bytesToRequest = min(I2S_READ_CHUNK_BYTES, remainingCapacity * 4);
    // Align to 8 bytes (two 32-bit samples) to ensure we always read aligned pairs of samples
    ...
    size_t bytesRead = 0;
    esp_err_t result = i2s_channel_read(s_rxHandle, s_readChunk,
    ...
    if (result == ESP_OK && bytesRead > 0) {
        ...
        if (!s_printedHex && samplesRead >= 5) {
            Serial.println(F("[Audio Debug] Raw 32-bit hex samples from I2S:"));
            for (int j = 0; j < 5; j++) {
                Serial.printf("  Sample %d: 0x%08X (signed: %d)\n", j, samples32[j], samples32[j]);
            }
            s_printedHex = true;
        }

        int16_t *dest16 = (int16_t *)(s_buffer + WAV_HEADER_SIZE_BYTES + s_pcmBytesWritten);
        ...
        // bytesToRequest is always 8-byte aligned (see above), so
        // samplesRead is always even; stepping by 2 is safe.
        for (size_t i = 0; i + 1 < samplesRead; i += 2) {
            // Decimate 16kHz -> 8kHz. INMP441 is MSB-aligned inside the
            // 32-bit register; shift right by 16 bits to map the
            // sign-extended data down to a 16-bit-range value.
            float raw0 = (float)(samples32[i] >> 16);
            float raw1 = (float)(samples32[i + 1] >> 16);
            ...
            if (absVal > LIMITER_THRESHOLD) {
                ...
                float compressed = LIMITER_THRESHOLD + (LIMITER_KNEE * over) / (over + LIMITER_KNEE);
...
const uint8_t *finalizeWav(size_t &outSize) {
    ...
    const size_t maxTrimBytes = (AUDIO_SAMPLE_RATE_HZ * (AUDIO_BITS_PER_SAMPLE / 8) * 150) / 1000;
    const size_t minTrimBytes = (AUDIO_SAMPLE_RATE_HZ * (AUDIO_BITS_PER_SAMPLE / 8) * 30) / 1000;
    const int16_t SILENCE_AMPLITUDE = 600; // below this is treated as "quiet enough to cut"
    ...
    if (s_pcmBytesWritten > maxTrimBytes) {
        ...
        if (trimSamples < minTrimSamples) trimSamples = minTrimSamples;
        if (trimSamples > maxTrimSamples) trimSamples = maxTrimSamples;
    ...
    const size_t MIN_SPEECH_BYTES = (AUDIO_SAMPLE_RATE_HZ * (AUDIO_BITS_PER_SAMPLE / 8) * 400) / 1000; // 400ms
...

    {
        ...
        for (size_t i = 0; i < totalSamples; i++) {
            int16_t absVal = (pcmSamples[i] < 0) ? (int16_t)(-pcmSamples[i]) : pcmSamples[i];
